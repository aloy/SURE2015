...
```{r}
(groupMeans <- summarise(grouped, mean=mean(Attitude.Score)))
summarise(groupMeans, mean.diff=diff(mean))
```
...
The step to building a ```trials``` data set is not conceptually difficult, but syntactically it's a little ugly. First, let's make a data set called resamples, made of 1000 resamples from ```grouped```. Do not print this data set. You will regret it. This may take awhile to execute, mostly because it has 480,000 rows.
